...
(function($, Drupal, window, document, undefined) {
  ...
  function side_menu_responsive() {
    var w = window.innerWidth || document.documentElement.clientWidth;
    // Mobile (No toggles).
    if (w < desktop_breakpoint && is_menu_desktop) {
    ...
    // Desktop (Toggles).
    else if (w >= desktop_breakpoint && !is_menu_desktop) {
    ...
    attach: function(context, settings) {
      ...
      if ($slider.length > 0) {
        ...
        if ($slider.children().length > 1) {
          ...
          $('.slider-play', $slider.parent).click();
...
(function($, Drupal, window, document, undefined) {
  ...
  Drupal.behaviors.contact_form_validation = {
    attach: function(context, settings) {

      // Phone numbers are special.
      ...
      $(':required, .required').off('blur keydown', callback).on('blur keydown', callback);
    }
  };
